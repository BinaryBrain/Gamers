...
create table `chat_room_participants` (`room_id` INTEGER NOT NULL,`person_id` INTEGER NOT NULL);
...
alter table `chat_room_participants` add constraint `room_participant_person_fk` foreign key(`person_id`) references `people`(`id`) on update CASCADE on delete CASCADE;
alter table `chat_room_participants` add constraint `room_participant_room_fk` foreign key(`room_id`) references `chat_rooms`(`id`) on update CASCADE on delete CASCADE;
...
ALTER TABLE chat_room_participants DROP FOREIGN KEY room_participant_person_fk;
ALTER TABLE chat_room_participants DROP FOREIGN KEY room_participant_room_fk;
